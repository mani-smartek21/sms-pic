...
$tree = '<select class="sp-folder-filter">';
...
$report['output'] .= '<ul class="sp-media">';

// Folders List
if(dirname($path) != '/') {
	$report['output'] .= '<li class="sp-media-folder">';
	$report['output'] .= '<div>';
	$report['output'] .= '<div>';
...
if(isset($media['folders_list']) && count($media['folders_list'])) {
	foreach ($media['folders_list'] as $single_folder) {
		...
		$report['output'] .= '<div>';
		$report['output'] .= '<div class="sp-media-image">';
		$report['output'] .= '<span class="sp-media-title">' . $single_folder .'</span>';
...
if(isset($media['images']) && count($media['images'])) {
	foreach ($media['images'] as $image) {
		...
		$report['output'] .= '<li class="sp-media-item" data-src="'. JURI::root(true) . '/' . $path .'" data-path="'. $path .'">';
		$report['output'] .= '<div>';
		$report['output'] .= '<div>';
		$report['output'] .= '<div class="sp-media-image">';
		...
		$report['output'] .= '<span class="sp-media-title">' . $title .'</span>';
		$report['output'] .= '</div>';
		$report['output'] .= '</div>';
		$report['output'] .= '</div>';
		$report['output'] .= '</div>';
