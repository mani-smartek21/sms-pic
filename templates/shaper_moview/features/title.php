	...
	public function renderFeature() {
		...
		if($menuitem) {
			...
			if($params->get('enable_page_title', 0)) {
				...
				if (count($title) == 2) {
					$output .= '<div class="page-header-icon-wrap pull-left">';
						$output .= '<i class="moview-before-title-icon ' . trim($title[1]) . '"></i>';
					$output .= '</div>';

					$output .= '<div class="page-header-title-wrap pull-left">';
					$output .= '<h2 class="page-header-title">' . trim($title[0]) . '</h2>';
					if($page_subtitle) {
					...
					$output .= '<div class="page-header-title-wrap">';
					$output .= '<h2 class="page-header-title">' . trim($title[0]) . '</h2>';

					if($page_subtitle) {
						$output .= '<h3 class="page-subheader-title">'. $page_subtitle .'</h3>';
					}
