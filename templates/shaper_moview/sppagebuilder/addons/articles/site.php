...
class SppagebuilderAddonArticles extends SppagebuilderAddons {
    ...
    public function render() {
        ...
        $title_fontweight = (isset($this->addon->settings->title_fontweight) && $this->addon->settings->title_fontweight) ? $this->addon->settings->title_fontweight : '';
        ...
        if (count($items)) {
            ...
            foreach ($items as $key => $item) {
                ...
                $leading_item = ($key == 0) ? 'leading-item' : '';

                $output .= '<div class="sppb-col-sm-' . round(12 / $columns) . ' ' . $leading_item . '">';
                $output .= '<div class="sppb-addon-article">';

                $output .= '<div class="article-thumb">';
                if (!$hide_thumbnail) {

                    $item_image = '';
                    if ($key == 0 && isset($item->image_medium) && $item->image_medium) {
                        $item_image = $item->image_medium;
                    } else {
                        $item_image = $item->image_thumbnail;
                    }
                    ...
                    if ($item->post_format == 'gallery') {
                        ...
                        if (isset($item_image) && $item_image) {
                            $output .= '<a href="' . $item->link . '" itemprop="url"><img class="sppb-img-responsive" src="' . $item_image . '" alt="' . $item->title . '" itemprop="thumbnailUrl"></a>';
                        }
                    }
                }
                ...
                if ($show_author || $show_category || $show_date) {
                    ...
                    $output .= '<i class="sp-moview-icon-clock"></i>';
                    if ($show_date) {
