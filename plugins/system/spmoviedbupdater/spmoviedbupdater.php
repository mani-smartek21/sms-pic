...
class  plgSystemSpmoviedbupdater extends JPlugin
{

    public function onExtensionAfterSave($option, $data) {

        if ( ($option == 'com_config.component') && ( $data->element == 'com_spmoviedb' ) ) {
            ...
            if(!empty($email) and !empty($license_key) )
            {
                ...
                $query = $db->getQuery(true)
                    ...
                    ->set($fields)
                    ->where($db->quoteName('name').'='.$db->quote('SP Movie Database'));
                $db->setQuery($query);
                $db->execute();
            }
        }
